...
import { getAreaTitle } from "@/lib/journey-copy";
...
export function ProgressDashboard() {
  ...
  return <div className="progress-page shell">
    ...
    <section className="evidence-section" aria-labelledby="area-record-title"><div className="section-heading"><div><p className="section-label">8つのエリア</p><h2 id="area-record-title">学びの記録</h2></div><p>完了数より、どんな操作を自分で再現できたかを見直せます。</p></div><ol className="progress-area-list">{journeyAreas.map((area) => { const missions = journeyMissions.filter((mission) => mission.areaId === area.id); const count = missions.filter((mission) => completed.has(mission.id)).length; const areaTitle = getAreaTitle(area, environment); return <li key={area.id}><span>{String(area.order).padStart(2, "0")}</span><div><h3>{areaTitle}</h3><div className="quiet-progress"><span style={{ width: `${(count / 4) * 100}%` }} /></div></div><strong>{count} / 4</strong><Link href={`/journey/${environment}/area/${area.id}`}>見る<span className="sr-only">：{areaTitle}</span></Link></li>; })}</ol></section>
